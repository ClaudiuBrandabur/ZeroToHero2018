...
class AppleColorPrint implements Printer<Apple> {

    @Override
    public void print(Apple apple) {
        System.out.println("An apple of " + apple.getColor() + " grams");
    }
}

class AppleWeightPrint implements Printer<Apple> {
...
class PrettyPrint implements Printer {

    @Override
    public void print(Object apple) {
        if(apple instanceof Apple){
            Apple ap = (Apple) apple;
        }
    }
}